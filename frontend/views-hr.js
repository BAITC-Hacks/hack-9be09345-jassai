...
export function launcherSettingsView(data={}){
  const connected=['connected','ready'].includes(data.ai_status);
  return `${heading('OpenAI через launcher','Ключ и модель настраиваются на этом компьютере при запуске приложения.')}<section class="panel launcher-panel"><div class="section-heading"><h2>Состояние приложения</h2><span class="pill ${connected?'success':'warn'}">${icon(connected?'check-circle':'info')} ${connected?'AI-модуль подключён':'AI-модуль не подключён'}</span></div><p class="explanation">${connected?'Это статус модуля в текущем процессе. Доступность ключа, модели и лимитов подтверждается успешным подбором рекомендаций.':'Для AI-рекомендаций настройте действующий ключ OpenAI и доступную вашему API-проекту модель.'}</p><h3>Как подключить или заменить ключ</h3><ol class="launcher-steps"><li>Остановите приложение: нажмите <kbd>Ctrl+C</kbd> в окне launcher.</li><li>Откройте терминал в папке скачанного репозитория и выполните <code>.\\launcher.cmd /configure-ai</code>.</li><li>Введите ключ в скрытом поле launcher и укажите идентификатор модели. Launcher проверит подключение коротким запросом без данных сотрудников.</li><li>После успешной проверки launcher запустит приложение и откроет браузер. Войдите снова; загруженные данные и прогресс сохраняются.</li></ol><p class="explanation">Ключ сохраняется средствами защиты Windows для текущего пользователя. В браузере вводить его не требуется. Если проверка не прошла, проверьте сеть, баланс API и доступ к модели.</p></section><section class="panel table-panel"><h2>Работа без AI</h2><p>При первом запуске можно оставить ключ пустым. Профиль, каталог, история и достижения продолжат работать; подбор AI покажет причину недоступности.</p><p>Чтобы удалить сохранённый ключ, остановите приложение и выполните <code>.\\launcher.cmd /disable-ai</code>. Launcher запросит подтверждение; данные сотрудников и прогресс сохранятся.</p><a class="button text" href="#home">Вернуться к рабочему пространству ${icon('arrow-right')}</a></section>`;
}

export function settingsView(data={}){
  if(data.not_connected)return launcherSettingsView(data);
