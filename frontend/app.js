...
async function renderRoute(){
  ...
  try{
    ...
    else if(raw==='settings'){if(!hr)throw new ApiError('Настройки доступны HR.',403);let data;try{data=await api('/api/hr/settings/ai');}catch(error){if(error.status!==404)throw error;data={not_connected:true,ai_status:state.status.ai_status};}if(version!==state.routeVersion)return;state.settings=data;html=view.settingsView(data);}
    ...
    if(version===state.routeVersion){main.innerHTML=html;if(raw==='settings'&&preview){for(const field of main.querySelectorAll('.provider-form input,.provider-form button,#ai-policy-form input,#ai-policy-form select,#ai-policy-form button'))field.disabled=true;}}
