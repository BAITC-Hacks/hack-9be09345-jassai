...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    }else if(action==='validate-starter-import'){
      requireRole('hr');const version=++state.importVersion,target=document.querySelector('#import-result');if(!target)return;state.batch=null;target.innerHTML=view.loadingView('Сравниваем стартовый набор с текущими данными…');importStage(2);
      try{const result=await api('/api/hr/import/starter',{method:'POST',body:{},timeout:20000});if(session!==state.session||version!==state.importVersion||!target.isConnected)return;state.batch=result.valid&&result.batch_id?{batch_id:result.batch_id,mode:'add'}:null;target.innerHTML='<p class="muted">Стартовый набор организаторов · режим добавления</p>'+view.importResult(result);importStage(state.batch?3:2);}
      catch(error){if(session!==state.session||version!==state.importVersion||!target.isConnected)return;state.batch=null;target.innerHTML=view.errorView(error.status===503?new ApiError('Стартовый набор не найден в установке приложения. Выберите четыре файла вручную.'):error);importStage(2);if(error.status===401){resetPrivateState();await renderRoute();}}
