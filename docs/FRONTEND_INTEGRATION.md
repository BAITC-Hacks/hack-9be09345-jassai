...
В main подключён OpenAI-провайдер B и защищённая настройка ключа через `launcher.cmd /configure-ai`. Пока HTTP-маршрутов настроек нет, интерфейс показывает инструкцию launcher. NVIDIA и резервный подбор не подключены. `/ready.ai_configured` означает наличие callable, а не успешную проверку ключа. `reason` B отображается в раскрывающемся обосновании; факторы и прирост берутся из ответа A.
...
Проверки интерфейса: `node --test tests/*.test.mjs`. Node нужен только разработчику для тестов. Текущая интеграционная проверка: [../TEST_RESULTS.md](../TEST_RESULTS.md).
